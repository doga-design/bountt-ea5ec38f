...
-- Fix 1: NULL-safe safety-net DELETE in create_expense_with_splits (newer version with p_expense_type)
...
-- Fix 1: NULL-safe safety-net DELETE in edit_expense (newer version with p_expense_type)
...
BEGIN
  ...
  WHERE expense_id = p_expense_id
    AND (
      (v_old_expense.paid_by_user_id IS NOT NULL AND user_id = v_old_expense.paid_by_user_id)
      OR (v_old_expense.paid_by_user_id IS NULL AND user_id IS NULL AND member_name = v_old_expense.paid_by_name)
    );

...
-- Phase 1: Add expense_type column to expenses table
ALTER TABLE public.expenses 
ADD COLUMN expense_type text NOT NULL DEFAULT 'split';

-- Add check constraint
ALTER TABLE public.expenses 
ADD CONSTRAINT expenses_expense_type_check CHECK (expense_type IN ('split', 'cover'));

-- Update create_expense_with_splits RPC to accept p_expense_type
CREATE OR REPLACE FUNCTION public.create_expense_with_splits(
  ...
  p_created_by uuid, 
  p_splits jsonb,
  p_expense_type text DEFAULT 'split'
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Insert expense with expense_type
  ...
  -- Log activity with expense_type in snapshot
...
-- Update edit_expense RPC to accept p_expense_type
...
BEGIN
  ...
  -- Update expense including expense_type
  UPDATE expenses
  ...
  -- Build snapshot for all change entries with expense_type
